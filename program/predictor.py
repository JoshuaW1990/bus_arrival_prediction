...
def preprocess_dataset(baseline_result, segment_df, route_stop_dist, trips):
    ...
    :return: 
    """
    result = pd.DataFrame(columns=['weather', 'rush_hour', 'baseline_result', 'delay_current_trip', 'delay_prev_trip', 'actual_arrival_time'])
    ...
    for i in xrange(len(baseline_result)):
        ...
        result.loc[len(result)] = [single_record.get('weather'), single_record.get('rush_hour'), single_record.get('estimated_arrival_time'), delay_current_trip, delay_prev_trip, single_record.get('actual_arrival_time')]
