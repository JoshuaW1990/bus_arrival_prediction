...
def compare_model(filename, path):
    ...
    output.plot(kind='scatter', y='baseline', x=range(0.0, 2.0, 0.001), xlim=(0, 2), ylim=(0, 2), ax=axes[0,0])
    output.plot(kind='scatter', y='linear_regression', x='actual_arrival_time', xlim=(0, 2), ylim=(0, 2),
                ax=axes[0, 1])
    output.plot(kind='scatter', y='SVM', x='actual_arrival_time', xlim=(0, 2), ylim=(0, 2), ax=axes[1, 0])
    output.plot(kind='scatter', y='NN', x='actual_arrival_time', xlim=(0, 2), ylim=(0, 2), ax=axes[1, 1])
...
def calculate_MSE(filename):
    ...
    true_ratio = [1.0] * len(output)
    mse_dict = dict()
    for column in columns:
        mse_dict[column] = MSE(true_ratio, output[column])
    return mse_dict
    # result = dict()
    # output = pd.read_csv(path + filename)
